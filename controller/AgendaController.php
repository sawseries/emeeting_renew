    ...
    public function insert_root() {
        ...
        if ($check['cnt'] <= 0) {
            if ($type == '2') {
                ...
                if ($_FILES['txt_file_root']['size'] > 0) {
                     $extension = pathinfo($_FILES['txt_file_root']['name'], PATHINFO_EXTENSION);
                    $file_tmp = $_FILES['txt_file_root']['tmp_name'];
                    $file_type = $_FILES['txt_file_root']['type'];
                    $file_name = rand() . "." . $extension;
                    $name = $_FILES['txt_file_root']['name'];
                    if(move_uploaded_file($file_tmp, "./storage/agenda/" . $file_name)){
                            $sqls = "insert into file set code='" . $code . "',doc_code='" . $_POST["hdndoc_code"] . "',"
                                    . "file='" . $file_name . "',"
                                    . "name='" . $name . "',"
                                    . "ip='" . Base::ip() . "'";
                            $insert = Base::query($sqls)->insert();
    ...
    public function insert_sub() {
       ...
       if ($check['cnt'] <= 0) {
            ...
            if ($type == '2') {
                ...
                if ($_FILES['txt_sub_file']['size'] > 0) {
                    $extension = pathinfo($_FILES['txt_sub_file']['name'], PATHINFO_EXTENSION);
                    $file_name = rand() . "." . $extension;
                    $file_tmp = $_FILES['txt_sub_file']['tmp_name'];
                    $name = $_FILES['txt_sub_file']['name'];
                    if (move_uploaded_file($file_tmp, "./storage/agenda/" . $file_name)) {
        ...
        if ($term->num_rows > 0) {
            ...
            foreach ($term as $terms) {
                ...
                if ($terms["type"] == '3') {
                    ...
                    $this->strsub .= "<a href='./storage/agenda/" . $file["file"] . "' target='_blank' title='เอกสารประกอบ'><img src='./assets/image/file_icon.jpg?key=<?php echo time();' style='width:20px;'></a>";
        ...
        if ($term->num_rows > 0) {
            ...
            foreach ($term as $terms) {
                ...
                if ($terms["type"] == '1') {
                    ...
                    $topic .= "<a href='./storage/agenda/" . $file["file"] . "' title='" . $file["name"] . "'  target='_blank' style='float:left;color:#1E90FF;'>";
                    if (!empty($terms["number"])) {
                        $topic .= $terms["number"] . ".";
                    }
                    $topic .= $terms["topic"];
...
//deletefile_present
    ...
    public function edit_root() {
        ...
        if ($type == '1') {
           ...
           $file_chk = Base::query("select * from file where code='" . $_POST["hdn_edit_root_code"] . "'")->fetchAll();
            if ($_FILES['txt_file_edit_root']['size'] > 0) {
            foreach ($file_chk as $file_chk) {
                if (file_exists("./storage/agenda/" . $file_chk["file"])) {
                    unlink("./storage/agenda/" . $file_chk["file"]);
                    $delete = Base::query("delete from file where id='" . $file_chk["id"] . "'")->execute(); //delete root
                }
            }
            
            $extension = pathinfo($_FILES['txt_file_edit_root']['name'], PATHINFO_EXTENSION);
            $file_name = rand() . "." . $extension;
            $file_tmp = $_FILES['txt_file_edit_root']['tmp_name'];
           $name = $_FILES['txt_file_edit_root']['name'];
...
//update file_present
    ...
    public function edit_sub() {
        ...
        if ($type == '1') {
            ...
            self::deletefile_edit($_POST["hdn_edit_sub_code"]);
             ...
             $file_chk = Base::query("select * from file where code='" . $_POST["hdn_edit_sub_code"] . "'")->fetchAll();
            if ($_FILES['txt_file_edit_sub']['size'] > 0) {
            foreach ($file_chk as $file_chk) {
                if (file_exists("./storage/agenda/" . $file_chk["file"])) {
                    unlink("./storage/agenda/" . $file_chk["file"]);
                    $delete = Base::query("delete from file where id='" . $file_chk["id"] . "'")->execute(); //delete root
                ...
                $extension = pathinfo($_FILES['txt_file_edit_sub']['name'], PATHINFO_EXTENSION);
                $file_name = rand() . "." . $extension;
                $file_tmp = $_FILES['txt_file_edit_sub']['tmp_name'];
                $name = $_FILES['txt_file_edit_sub']['name'];
    ...
    function getitmfile() {
        $terms = Base::query("select * from file where code ='" . $_POST['code'] . "'")->one();

        echo "<div id='itm_file_" . $terms['id'] . "' style='padding:0.5em;background-color:#E6E6FA;margin:0.5em;'>"
        . "<table style='width:100%;'>"
        . "<tr><td style='width:90%;'>"
        . "<a href='./storage/agenda/" . $terms["file"] . "' target='_blank'>" . $terms["name"] . "</a>"
        . "</td>"
        . "<td style='width:10%;text-align:center;color:red;'>"
        . "<a onclick='deleteitmfile(" . $terms["id"] . ");' style='color:red;cursor:pointer;'>ลบ</a>"
        . "</td>"
        . "</table>"
        . "</div>";
    }
